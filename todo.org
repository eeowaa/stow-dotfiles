...
** TODO Ensure proper trailing newline mechanics between Vim and Emacs
Emacs uses ~require-final-newline~ and Vim uses ~fixendofline~. Though both are
enabled, sometimes I see EOL changes at the end of files with =git diff=.
